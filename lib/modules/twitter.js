...
oauthModule.submodule('twitter')
  ...
  .fetchOAuthUser( function (accessToken, accessTokenSecret, params) {
    ...
    this.oauth.get(this.apiHost() + '/1/users/show.json?user_id=' + params.user_id, accessToken, accessTokenSecret, function (err, data) {
  ...
  .convertErr( function (data) {
    data = data.data.match(/<error>(.+)<\/error>/);
    data = (data && data.length > 1) ? data[1] : "Twitter Oauth [Could not parse error.]"; 
